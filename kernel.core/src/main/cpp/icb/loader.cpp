...
static Java::ClassError loadClass(void* arg, Java::VM& vm, GC::Root<Java::Thread>& thread, GC::Root<Java::ClassFile>& root, const GC::Root<char>& name)
{
	...
	while (loader.m_bundle.next(entry))
	{
		if (equals(name, entry.name))
			return vm.defineClass(root, thread, entry.content, entry.length);
	}
